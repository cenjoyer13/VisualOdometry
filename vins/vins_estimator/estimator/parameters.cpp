...
// readParam<T>() was removed with the ROS dependency: it pulled parameters
// from the ROS parameter server via a ros::NodeHandle and was never
// instantiated here -- every setting comes from the yaml.
...
#ifndef VINS_WITH_CUDA
    if (USE_GPU || USE_GPU_ACC_FLOW)
        ROS_WARN("built without CUDA OpenCV: forcing use_gpu/use_gpu_acc_flow to 0");
    USE_GPU = 0;
    USE_GPU_ACC_FLOW = 0;
#endif
